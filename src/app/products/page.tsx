        ...
        <ul className="flex flex-wrap justify-center items-center gap-x-8 gap-y-4">
          {menuItems.map((item) => (
            <li key={item.name}>
              <Link 
                href={item.href}
                className="group relative text-[12px] font-bold uppercase tracking-[0.2em] text-slate-500 hover:text-slate-950 transition-colors"
              >
                {item.name}
      ...
      <section className="bg-slate-50 py-20 border-t border-slate-200">
        <div className="max-w-4xl mx-auto text-center px-6">
          <h3 className="text-3xl font-light text-slate-900 mb-8">
            ...
            <span className="italic font-serif font-semibold">We offer custom solutions for every window.</span>
          </h3>
          <Link href="/contact">
            <button className="bg-slate-900 text-white px-10 py-4 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-orange-500 transition-colors duration-300">
